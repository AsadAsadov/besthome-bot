...
def handle_admin_db_upload(message):
    ...
    if not doc or not doc.file_name or not doc.file_name.lower().endswith(".db"):
        bot.send_message(
            chat_id,
    ...
    if doc.file_name != "besthome.db":
        try:
            bot.send_message(chat_id, "ℹ️ Fayl adı besthome.db deyil, davam olunur.")
        except Exception:
            pass
