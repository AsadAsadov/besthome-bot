    ...
    def __getattr__(self, name):
        ...
        if name.startswith("send_"):
            ...
            def wrapped(*args, **kwargs):
                ...
                editable = name == "send_message" and isinstance(
                    reply_markup, types.InlineKeyboardMarkup
                )
                _track_sent_message(chat_id, result, editable=editable)
                return result
...
SUPPORT_THREAD_STATUS_CLOSED = "CLOSED"
...
def _record_last_bot_message(
    ...
    user_last_message[chat_id] = {"message_id": message_id, "editable": editable}
...
def _track_sent_message(
    chat_id: Optional[int], result: Any, *, editable: bool
) -> None:
    message_id = _extract_message_id_from_result(result)
    if message_id:
        _record_last_bot_message(chat_id, message_id, editable)
...
def _edit_last_message_reply_markup(proxy: _BotProxy, edit_func, *args, **kwargs):
    ...
    if chat_id is None:
        return edit_func(*args, **kwargs)
    last = user_last_message.get(chat_id)
    if not last or not last.get("editable"):
        return None
    ...
    if message_id:
        ...
        _record_last_bot_message(chat_id, message_id, editable=True)
        return result
...
def _support_row_to_dict(row: Optional[sqlite3.Row]) -> Optional[dict]:
    if not row:
        return None
    return {key: row[key] for key in row.keys()}

def get_support_thread_by_user(user_id: int) -> Optional[dict]:
    conn = get_local_conn()
    cur = conn.cursor()
    cur.execute("SELECT * FROM support_threads WHERE user_id=?", (user_id,))
    row = cur.fetchone()
    conn.close()
    return _support_row_to_dict(row)

def upsert_support_thread(
    user_id: int,
    *,
    status: str,
    needs_reply: int,
    ...
    cur.execute(
        ...
        ON CONFLICT(user_id) DO UPDATE SET
            ...
            unread_for_admin=excluded.unread_for_admin,
...
def update_support_thread(user_id: int, **fields: Any) -> Optional[dict]:
    allowed_fields = {
        "status",
        "needs_reply",
        "unread_for_admin",
    ...
    if not updates:
        return get_support_thread_by_user(user_id)
    assignments = []
    params = []
    ...
    cur = conn.cursor()
    cur.execute(
        f"UPDATE support_threads SET {', '.join(assignments)} WHERE user_id=?",
        params,
    )
    conn.commit()
    cur.execute("SELECT * FROM support_threads WHERE user_id=?", (user_id,))
...
def ensure_support_thread(user_id: int, status: str) -> Optional[dict]:
    existing = get_support_thread_by_user(user_id)
    if existing:
        if status and existing.get("status") != status:
            return update_support_thread(user_id, status=status)
        return existing
    return upsert_support_thread(
        user_id,
        status=status,
        needs_reply=0,
        unread_for_admin=0,
...
def add_support_message(thread_id: int, sender: str, text: Optional[str]) -> None:
    conn = get_local_conn()
    cur = conn.cursor()
    cur.execute(
        """
        ...
        VALUES (?, ?, ?)
        """,
        (thread_id, sender, text),
    )
    conn.commit()
    conn.close()

def list_support_threads() -> List[dict]:
    conn = get_local_conn()
    cur = conn.cursor()
    cur.execute(
        """
...
def list_support_messages(thread_id: int, limit: int = 20) -> List[dict]:
    conn = get_local_conn()
    cur = conn.cursor()
    cur.execute(
        """
    ...
    rows = cur.fetchall()
    conn.close()
    messages = [{key: row[key] for key in row.keys()} for row in rows]
    messages.reverse()
    return messages
...
def get_bonus_probabilities() -> Dict[int, int]:
    ...
    rows = cur.fetchall()
    conn.close()
    allowed_days = set(BONUS_DEFAULT_PROBABILITIES.keys())
    if not rows:
    ...
    for day in allowed_days:
        weights.setdefault(day, BONUS_DEFAULT_PROBABILITIES[day])
    if weights != BONUS_DEFAULT_PROBABILITIES:
        update_bonus_probabilities(BONUS_DEFAULT_PROBABILITIES)
        return dict(BONUS_DEFAULT_PROBABILITIES)
    return weights
...
def update_bonus_probabilities(new_weights: Dict[int, int]) -> Dict[int, int]:
    ...
    for day, weight in new_weights.items():
        try:
            ...
            continue
    normalized = {day: weight for day, weight in normalized.items() if day in allowed_days}
    if not normalized:
...
def bonus_stats_period_range(period: str) -> Tuple[datetime, datetime, str]:
    start_date, end_date, label = stats_period_range(period)
    start_dt = datetime.combine(start_date, datetime.min.time())
    end_dt = datetime.combine(end_date + timedelta(days=1), datetime.min.time())
    return start_dt, end_dt, label

def fetch_bonus_stats(period: str) -> dict:
    ...
    period_spins, period_days = cur.fetchone()

    cur.execute(
        """
        SELECT granted_days, COUNT(*)
        FROM chance_bonus_logs
        WHERE created_at >= ? AND created_at < ?
        GROUP BY granted_days
        """,
        (start_dt.isoformat(), end_dt.isoformat()),
    )
...
def pick_bonus_days() -> int:
    ...
    allowed_days = set(BONUS_DEFAULT_PROBABILITIES.keys())
    filtered = {day: weight for day, weight in probabilities.items() if day in allowed_days}
    if not filtered:
        filtered = dict(BONUS_DEFAULT_PROBABILITIES)
    options = list(filtered.keys())
    weights = [max(0, int(filtered[o])) for o in options]
    if not any(weights):
...
def can_use_chance(record: Optional[dict], now: datetime) -> Tuple[bool, Optional[datetime]]:
    ...
    if not last_used_at:
        return True, None
    next_available = last_used_at + timedelta(hours=BONUS_SPIN_COOLDOWN_HOURS)
...
def get_support_admin_active_thread_user_id(admin_id: int) -> Optional[int]:
    user_id = support_admin_state.get(admin_id, {}).get("active_thread_user_id")
    return int(user_id) if user_id else None

def get_support_admin_reply_user_id(admin_id: int) -> Optional[int]:
    user_id = support_admin_state.get(admin_id, {}).get("reply_to_user_id")
    return int(user_id) if user_id else None

def _set_support_admin_state(admin_id: int, **updates: Optional[Union[str, int]]):
    state = support_admin_state.get(admin_id, {})
    state.update(updates)
    support_admin_state[admin_id] = state

def _should_notify_admin_for_user(user_id: int) -> bool:
    now_ts = time.time()
    last_ts = support_last_admin_notify_ts.get(user_id, 0.0)
    if now_ts - last_ts < 30:
        return False
    support_last_admin_notify_ts[user_id] = now_ts
    return True
...
def _is_inline_keyboard(
    keyboard: Optional[Union[types.InlineKeyboardMarkup, types.ReplyKeyboardMarkup]],
) -> bool:
    return isinstance(keyboard, types.InlineKeyboardMarkup)

def _clear_last_message_record(user_id: int) -> None:
...
def _store_last_message_record(user_id: int, message_id: int, editable: bool) -> None:
    last_ui_message_id[user_id] = message_id
    user_last_message_id[user_id] = message_id
    user_last_message[user_id] = {"message_id": message_id, "editable": editable}
...
def render_ui(
    ...
    *,
    ...
    can_edit = (
        prefer_edit
        and last
        and last.get("editable")
        and (_is_inline_keyboard(keyboard) or keyboard is None)
    )
    if can_edit:
        message_id = user_last_message_id.get(user_id)
        if message_id:
            try:
                real_bot = bot._bot if getattr(bot, "_bot", None) is not None else bot
                real_bot.edit_message_text(
                    text,
                    target_chat_id,
                    message_id,
                    reply_markup=keyboard,
                ...
                _store_last_message_record(user_id, message_id, editable=True)
                return message_id
            except Exception as exc:
                if "message is not modified" in str(exc).lower():
                    _store_last_message_record(user_id, message_id, editable=True)
                    return message_id
                logger.debug("render_ui edit failed chat_id=%s", target_chat_id)
    try:
        ...
        logger.exception("render_ui send failed chat_id=%s", target_chat_id)
        return None
    editable = _is_inline_keyboard(keyboard)
    _store_last_message_record(user_id, msg.message_id, editable=editable)
...
def update_ui_message(
    ...
    *,
    ...
    if message_id and last_user and message_id < last_user:
        ...
        message_id = None
        _clear_last_message_record(user_id)
...
def build_search_menu_inline_markup(chat_id: int) -> types.InlineKeyboardMarkup:
    mk = types.InlineKeyboardMarkup()
    mk.row(
    ...
    mk.row(
        types.InlineKeyboardButton(
            "🔍 Açar sözlə axtar", callback_data="search_menu:keyword"
        ),
        types.InlineKeyboardButton(
...
def handle_user_stats_callback(c):
    ...
    try:
        ...
        pass
    if c.message:
        send_user_statistics(chat_id, period)
...
def render_support_ui(
    ...
    *,
    ...
    can_edit = (
        last
        and last.get("editable")
        and (keyboard is None or isinstance(keyboard, types.InlineKeyboardMarkup))
    )
    if can_edit:
        msg_id = render_ui(
            chat_id,
        ...
        if msg_id:
            if is_admin(chat_id):
                _set_support_admin_state(chat_id, message_id=msg_id)
            else:
                set_support_session_last_ui_message_id(chat_id, msg_id)
            return msg_id
...
@bot.callback_query_handler(func=lambda c: c.data and c.data.startswith("search_menu:"))
@callback_guard
def cb_search_menu(c):
    ...
    if action == "sale":
        ...
        handle_structured_search_menu(chat_id, "kir")
    elif action == "keyword":
        handle_keyword_search_menu(chat_id)
    elif action == "phone":
        handle_phone_search_menu(chat_id)
    elif action == "favs":
...
def handle_notifications_menu(chat_id: int):
    if not ensure_feature_available(chat_id, "notifications"):
        return
    if not check_subscription(chat_id):
        return
    set_navigation_state(chat_id, STATE_SEARCH_ACTION)
...
def show_notifications_inbox(chat_id: int, period: str, page: int = 1, message=None):
    ...
    if not listings:
        ...
        if message:
            render_ui(message.chat.id, text, mk_empty)
        else:
            render_ui(chat_id, text, mk_empty)
        return
...
def show_agent_notifications_inbox(chat_id: int, page: int = 1, message=None):
    ...
    if total == 0:
        ...
        text = "🔔 Müştəri istəyi bildirişi yoxdur."
        if message:
            render_ui(message.chat.id, text, mk_empty)
        else:
            render_ui(chat_id, text, mk_empty)
    ...
    header_text = "\n".join(header_lines)
    if message:
        render_ui(message.chat.id, header_text, mk)
    else:
        render_ui(chat_id, header_text, mk)
...
def send_criteria_list(chat_id: int, message=None):
    ...
    if not rows:
        ...
        text = "⚙️ Saxlanılmış kriteriya yoxdur."
        if message:
            render_ui(message.chat.id, text, mk)
        else:
            render_ui(chat_id, text, mk)
...
def show_keyword_alert_list(chat_id: int, page: int = 1, message=None):
    ...
    if total == 0:
        ...
        text = "🔔 Açar söz bildirişi yoxdur."
        if message:
            render_ui(message.chat.id, text, mk)
        else:
            render_ui(chat_id, text, mk)
    ...
    mk.add(types.InlineKeyboardButton("⬅️ Geri", callback_data="kw_alert_menu"))
    if message:
        render_ui(message.chat.id, header, mk)
    else:
        render_ui(chat_id, header, mk)
...
def show_keyword_alert_hits(chat_id: int, period: str, index: int = 0, message=None):
    ...
    if not listings:
        mk = types.InlineKeyboardMarkup()
        mk.add(types.InlineKeyboardButton("⬅️ Geri", callback_data="kw_hits_menu"))
        text = f"🔔 {period_label} üçün bildiriş elanları tapılmadı."
        if message:
            render_ui(message.chat.id, text, mk)
        else:
            render_ui(chat_id, text, mk)
...
def show_saved_notifications(message):
    ...
    handle_notifications_menu(message.chat.id)
...
def fetch_all_results(chat_id: int, mode: str, params: dict):
    ...
    if mode == "phone":
        items, total = query_phone_results(
            params.get("digits", ""), offset=0, limit=None
        )
        return items, len(items), None
    if mode == "favorites":
    ...
    if mode == "today":
        ...
        items, total = query_today_results(
            filters_copy, offset=0, limit=None
        )
...
def agents_panel(c):
    ...
    render_ui(chat_id, "🏢 Vasitəçi elanları axtarış sistemi:", mk)
...
def cb_agent_filter(c):
    ...
    render_ui(chat_id, "🔍 Vasitəçi elanları üçün filtr seç:", mk)
...
def _support_status_badge(thread: dict) -> str:
    if int(thread.get("unread_for_admin") or 0) == 1:
        return "🔴"
    if int(thread.get("needs_reply") or 0) == 1:
        return "🟡"
    if thread.get("status") == SUPPORT_THREAD_STATUS_ACTIVE:
        return "🟢"
    if thread.get("status") == SUPPORT_THREAD_STATUS_CLOSED:
        return "⚪"
    return "⚪"

def _support_status_label(thread: dict) -> str:
    status = thread.get("status") or SUPPORT_THREAD_STATUS_OPEN
    if int(thread.get("needs_reply") or 0) == 1:
        return f"{status} (cavab gözləyir)"
    return status
...
def format_support_inbox_text(threads: List[dict]) -> str:
    open_count = sum(
        1 for thread in threads if thread.get("status") == SUPPORT_THREAD_STATUS_OPEN
    )
    unread_count = sum(
        1 for thread in threads if int(thread.get("unread_for_admin") or 0) == 1
    )
    ...
    if not threads:
        return f"{header}\n\n❌ Heç bir sorğu yoxdur."
    return header

def build_support_inbox_markup(threads: List[dict]) -> types.InlineKeyboardMarkup:
    mk = types.InlineKeyboardMarkup(row_width=1)
    for thread in threads:
        label = _format_support_thread_label(thread)
        mk.add(
            types.InlineKeyboardButton(
                label, callback_data=f"supportinbox:open:{thread['user_id']}"
            )
        )
    mk.add(types.InlineKeyboardButton("⬅️ Geri", callback_data="ui_admin_menu"))
    return mk

def _truncate_support_text(text: Optional[str], limit: int = 140) -> str:
    if not text:
        return "-"
...
def _format_support_thread_messages(messages: List[dict]) -> str:
    if not messages:
        return "Mesaj yoxdur."
    lines = []
    for msg in messages:
...
def build_support_thread_view_markup(
    user_id: int, status: Optional[str]
) -> types.InlineKeyboardMarkup:
    mk = types.InlineKeyboardMarkup(row_width=2)
    if status == SUPPORT_THREAD_STATUS_CLOSED:
...
def show_support_inbox(chat_id: int, message: Optional[types.Message] = None):
...
def show_support_thread_view(
    ...
    *,
    ...
    if set_active:
        thread = update_support_thread(
            user_id,
            status=SUPPORT_THREAD_STATUS_ACTIVE,
            unread_for_admin=0,
        )
    messages = list_support_messages(thread["id"], limit=20) if thread else []
    header = _format_support_thread_header(thread)
...
def show_support_reply_prompt(
    chat_id: int, user_id: int, message: Optional[types.Message] = None
):
    mk = types.InlineKeyboardMarkup(row_width=1)
    mk.add(
        types.InlineKeyboardButton(
            "⬅️ Geri (Söhbət)", callback_data=f"supportinbox:open:{user_id}"
        )
    )
...
def send_admin_panel(
    chat_id: int,
    page: int = 1,
    text: str = TEXTS_AZ["admin_panel_title"],
    message: Optional[types.Message] = None,
):
...
def _handle_admin_panel_action(
    ...
    if action_text == TEXTS_AZ["admin_panel_pending_listings"]:
        show_pending_listings(chat_id, message=message)
    elif action_text == TEXTS_AZ["admin_panel_stats"]:
        show_admin_stats_menu(chat_id, message=message)
    elif action_text == TEXTS_AZ["admin_panel_bonus_stats"]:
        show_bonus_stats(chat_id, message=message)
    elif action_text == TEXTS_AZ["admin_panel_activity_stats"]:
        show_admin_activity_stats_menu(chat_id, message=message)
    elif action_text == TEXTS_AZ["admin_panel_customer_requests"]:
        show_customer_requests_overview(chat_id, "day", message=message)
    elif action_text == "📊 QR Statistikası":
        send_qr_stats_menu(chat_id, message=message)
    elif action_text == FINANCIAL_REPORTS_BUTTON:
        send_financial_reports_menu(chat_id, message=message)
        ...
        show_admin_promo_menu(chat_id, message=message)
    elif action_text == TEXTS_AZ["admin_panel_users"]:
        show_users_menu(chat_id, message=message)
    elif action_text == TEXTS_AZ["admin_panel_feature_flags"]:
        send_feature_flags_menu(chat_id, message=message)
    elif action_text == TEXTS_AZ["admin_panel_send_update"]:
        broadcast_bot_update(chat_id, message=message)
        ...
        show_support_inbox(chat_id, message=message)
    elif action_text == TEXTS_AZ["admin_panel_customer_requests_access"]:
        show_customer_requests_access_admin(chat_id, message=message)
    elif action_text == TEXTS_AZ["admin_panel_archived_requests"]:
        show_archived_requests(chat_id, page=1, message=message)
...
@bot.callback_query_handler(func=lambda c: c.data and c.data.startswith("supportinbox:"))
@callback_guard
def cb_support_inbox_admin(c):
    ...
    if action == "list":
        ...
        return
    if action == "open":
        user_id = int(c.data.split(":", 2)[2])
        show_support_thread_view(chat_id, user_id, message=c.message)
        safe_answer_callback_query(c.id)
        return
    if action == "reply":
        user_id = int(c.data.split(":", 2)[2])
        show_support_reply_prompt(chat_id, user_id, message=c.message)
        safe_answer_callback_query(c.id)
        return
    if action == "close":
        user_id = int(c.data.split(":", 2)[2])
...
def cb_admin_stats_menu(c):
    ...
    if action == "activity":
        ...
        return
    if action == "bonus":
        show_bonus_stats(chat_id, message=c.message)
        return
    if action == "qr":
...
def handle_bonus_probability_edit(message):
    ...
    if not new_weights:
        bot.send_message(
            message.chat.id,
            "⚠️ Düzgün format: 0=40,1=35,2=20,3=5",
        )
...
def send_qr_stats_menu(chat_id: int, message: Optional[types.Message] = None):
    if not is_admin(chat_id):
        return
...
def send_qr_stats(
    chat_id: int, range_key: str, message: Optional[types.Message] = None
):
    if not is_admin(chat_id):
        return
...
def cb_qr_stats(c):
    ...
    action = c.data.split(":", 1)[1]
    if action == "top":
        send_qr_top_areas(chat_id, message=c.message)
    else:
        send_qr_stats(chat_id, action, message=c.message)
...
def show_admin_customer_request_types(
    ...
    if not buttons:
        ...
        if message:
            render_ui(message.chat.id, text, None)
        else:
            render_ui(chat_id, text, None)
        return
...
def show_admin_requests_by_rayon(
    ...
    if message:
        render_ui(message.chat.id, footer, mk)
        return
    render_ui(chat_id, footer, mk)

def show_customer_requests_overview(
    chat_id: int, period: str = "day", message: Optional[types.Message] = None
):
...
def handle_financial_reports_menu(c):
    ...
    chat_id = c.message.chat.id
    if action == "history":
        show_payment_history_list(chat_id, page=1, message=c.message)
    elif action == "referral":
        show_referral_stats(chat_id, message=c.message)
    elif action == "monthly":
        show_revenue_report(chat_id, message=c.message)
    elif action == "back":
        page = admin_panel_page_state.get(chat_id, 1)
        send_admin_panel(chat_id, page=page, message=c.message)
...
def handle_admin_payments_menu(c):
    ...
    if len(parts) > 2:
        try:
            ...
            page = 1
    if section == "card":
        show_card_payments(c.message.chat.id, page=page, message=c.message)
    elif section == "manual":
        show_manual_payment_requests(c.message.chat.id, page=page, message=c.message)
    else:
...
def cb_admin_request_period(c):
    ...
    try:
        ...
        pass
    period = c.data.split(":", 1)[1]
    show_customer_requests_overview(c.message.chat.id, period, message=c.message)
...
def show_user_profile(chat_id: int, user_id: int):
    ...
    profile_text = "\n".join(
        [
            ...
            "🎁 Şans: 3 günə 1 dəfə",
            f"🚦 Son 3 gün istifadə: {'Bəli' if used_today else 'Xeyr'}",
        ]
    )
...
def start_direct_user_message_flow(
    chat_id: int, message: Optional[types.Message] = None
):
    if not is_admin(chat_id):
        return
...
def cb_admin(c):
    ...
    if cmd == "pending":
        ...
        show_admin_promo_menu(c.message.chat.id, message=c.message)
...
def cb_admin_promo(c):
    ...
    if action == "gen" and len(parts) > 2:
        ...
        mk = types.InlineKeyboardMarkup()
        mk.add(types.InlineKeyboardButton(TEXTS_AZ["admin_promo_back"], callback_data="adm|promos"))
        if code:
            text = TEXTS_AZ["admin_promo_created"].format(days=days, code=code)
        else:
        ...
        if len(parts) > 2:
            try:
                ...
                page = 1
        show_admin_promo_list(c.message.chat.id, page=page, message=c.message)
    elif action == "stats":
        page = 1
        if len(parts) > 2:
            try:
                ...
                page = 1
        show_admin_promo_stats(c.message.chat.id, page=page, message=c.message)
...
def admin_show_user_panel(
    ...
    if message:
        render_ui(admin_chat_id, info_txt, mk, parse_mode="HTML")
        return
    render_ui(admin_chat_id, info_txt, mk, parse_mode="HTML")

def show_payment_history_list(
    chat_id: int, page: int = 1, message: Optional[types.Message] = None
):
    if not is_admin(chat_id):
        return
...
def cb_pay_history_list(c):
    ...
    if len(parts) > 1:
        try:
            ...
            page = 1
    show_payment_history_list(c.message.chat.id, page=page, message=c.message)
    try:
        bot.answer_callback_query(c.id)
    except Exception:
        pass
...
def show_users_menu(chat_id, message: Optional[types.Message] = None):
    ...
    try:
        ...
        try:
            if message and not force_new:
                render_ui(chat_id, text, mk, parse_mode="HTML")
            else:
                render_ui(chat_id, text, mk, prefer_edit=False, parse_mode="HTML")
        except Exception:
            logger.error("Admin send failed", exc_info=True)
...
def show_pending_listings(chat_id, message: Optional[types.Message] = None):
    return
    conn = get_local_conn()
    cur = conn.cursor()
    cur.execute(
...
def cb_block_new_user(c):
    ...
    try:
        render_ui(c.message.chat.id, f"⛔ İstifadəçi bloklandı\nID: {uid}", None)
    except Exception:
...
def broadcast_bot_update(
    admin_chat_id: int, message: Optional[types.Message] = None
):
...
def handle_bot_refresh(message):
    ...
    admin_state.pop(chat_id, None)
    ui_message_state.pop(chat_id, None)
...
def show_agent_my_customers(
    ...
    if total == 0:
        ...
        text = "👥 Hələ müştəri seçməmisiniz."
        if message:
            render_ui(message.chat.id, text, mk_empty)
        else:
            render_ui(chat_id, text, mk_empty)
...
def handle_stats_period_callback(c):
    ...
    if c.message:
        show_admin_stats(chat_id, period=period)

@bot.callback_query_handler(
    func=lambda c: c.data and c.data.startswith("bonus_stats_period:")
)
@callback_guard
def handle_bonus_stats_period_callback(c):
    period = c.data.split(":", 1)[1] if c.data else "day"
    if period not in STATS_PERIOD_MAP:
        period = "day"
    ...
    if c.message:
        show_bonus_stats(chat_id, period=period)

def show_referral_stats(chat_id: int, message: Optional[types.Message] = None):
    if not is_admin(chat_id):
        return
...
def bonus_stats_period_keyboard(selected: str) -> types.InlineKeyboardMarkup:
    mk = types.InlineKeyboardMarkup()
    buttons = [
        types.InlineKeyboardButton(
            TEXTS_AZ["admin_stats_period_day"], callback_data="bonus_stats_period:day"
        ),
        types.InlineKeyboardButton(
            TEXTS_AZ["admin_stats_period_week"], callback_data="bonus_stats_period:week"
        ),
        types.InlineKeyboardButton(
            TEXTS_AZ["admin_stats_period_month"],
            callback_data="bonus_stats_period:month",
        ),
    ]
    mk.row(*buttons)
...
def show_bonus_stats(
    ...
    period = period or admin_bonus_stats_period.get(chat_id, "day")
    if period not in STATS_PERIOD_MAP:
        period = "day"
    admin_bonus_stats_period[chat_id] = period

    stats = fetch_bonus_stats(period)
    probabilities = get_bonus_probabilities()
    ...
    lines.append("📊 Ümumi göstəricilər")
    lines.append(f"• Ümumi cəhd: {total_spins}")
    lines.append(f"• 0 gün: {counts.get(0, 0)}")
...
def show_revenue_report(chat_id: int, message: Optional[types.Message] = None):
    if not is_admin(chat_id):
        return
...
@bot.message_handler(
    content_types=["text"],
    func=lambda m: is_admin(m.chat.id) and get_support_admin_reply_user_id(m.chat.id),
)
def support_admin_message_router(message):
    ...
    update_support_thread(
        reply_user_id,
        status=SUPPORT_THREAD_STATUS_WAITING,
        needs_reply=0,
        unread_for_admin=0,
        last_message_from="admin",
        last_message_at=now_utc().isoformat(),
