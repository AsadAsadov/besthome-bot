...
def delete_user_command_message(message: types.Message) -> None:
    if (
        ...
        and is_user_support_active(message.chat.id)
        and is_menu_action(message)
    ):
...
def is_menu_action(message: Union[types.Message, types.CallbackQuery]) -> bool:
    if isinstance(message, types.CallbackQuery):
        return True
    return bool(
        message.text and is_main_menu_button_text(message.chat.id, message.text)
    )
...
def cb_admin_stats_menu(c):
    ...
    if action == "overall":
        ...
        show_admin_stats(chat_id, message_id=c.message.message_id)
        return
    if action == "activity":
...
def handle_bonus_stats_period_callback(c):
    ...
    if c.message:
        show_bonus_stats(chat_id, period=period, message_id=c.message.message_id)
